...
\paragraph*{3.} Explique porquê não podemos calcular: 
    ...
    \paragraph*{4.} Encontre o domínio das seguintes funções:
    \begin{enumerate}[a)]
...
\textbf{4.} a) $\text{Dom } f  = \{x\in\mathbb{R} \mid x\ge 4\}$; b)  $\text{Dom } g  = \left\{x\in\mathbb{R} \mid x \neq \tfrac{3}{2}\right\} $; c)  $\text{Dom } h  = \{x\in\mathbb{R} \mid x > 1\}$.  
